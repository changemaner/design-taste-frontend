# 模式词汇表（Pattern Vocabulary）—— SKILL.md §10 外置参考

> 触发时机：§4/§5/§11 引用布局模式名、或需向用户准确描述布局方案时读取；用既有命名，勿自造近义词。



This is a vocabulary, not a library. The agent should KNOW these pattern names to communicate about them, design with them in mind, and reach for them when the design read calls for them. **Code sketches for motion patterns live in `reference/motion-skeletons.md`; page-section layouts are built per-project from Section 4 directives (no generic block library, by design).**
